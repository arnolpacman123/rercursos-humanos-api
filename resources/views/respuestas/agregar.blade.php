            ...
            <div class="col-lg-12">
                <div class="ibox ">
                    <div class="ibox-title">
                        <a class="btn btn-primary" href="{{route('respuetas.index')}}">Listado</a>
                        <div class="ibox-tools"><a class="collapse-link"><i class="fa fa-chevron-up"></i></a></div>
                    </div>
                    <div class="ibox-content">
                        <form action="{{route("respuestas.insertar")}}" method="post">
                            @csrf
                            
                            <div class="form-group row">
                                <label class="col-sm-2 col-form-label">Respuesta:<i class="text-danger">*</i></label>
                                <div class="col-sm-10"><input type="text" class="form-control" name="respuesta" value="{{old('respuesta')}}" required=""></div>
                            </div>
                            @error('respuesta')
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10"><input type="number" class="form-control" name="valor" value="{{old('valor')}}" required=""></div>
                            </div>
                            @error('valor')
                                <div class="alert alert-danger alert-dismissable">{{$message}}<button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button></div>
                            @enderror
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control" name="pregunta_id"  id="pregunta_id">
                                        ...
                                        @foreach ($preguntas as $pregunta)
                                            <option value="{{$pregunta->id}}" @if(old('pregunta_id')==$pregunta->id) selected="" @endif>{{$pregunta->pregunta}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
